# python/src/auto_commit/output/formatter.py
...
class BaseFormatter(ABC):
    """Abstract base formatter, implemented by JSON and console formatters."""

    @abstractmethod
    def emit(self, payload: OutputPayload) -> None:
        """Render the payload to stdout/stdout using the chosen format."""
        ...
...
class ConsoleFormatter(BaseFormatter):
    """
    Formatter that prints a concise human summary.

    Detailed Rich UI remains in the CLI modules; this only prints a final
    summary line so JSON mode and console mode can share the same payload.
    """
...
def get_formatter(
    ...
    """
    Factory returning the appropriate formatter.

    quiet or json_mode → JSON only
    otherwise → human console summary
    """
